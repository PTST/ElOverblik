...
class ElOverblik(object):
    ...
    def get_meter_readings(self, date_start, date_end):
        ...
        data = [MeterReading(x["readingDate"], x["meterReading"], x["measurementUnit"]) for x in _json]
    ...
    def get_meter_time_series(self, date_start, date_end, aggregation):
        ...
        if aggregation not in self.aggregations_types:
            raise ValueError(f"Aggregations is not allowed, allowed aggregations are: {str(allowed_aggregations)}")
        ...
        for series in _json:
            ...
            for period in series["Period"]:
                ...
                data += [MeterPeriodReading(start, end, x["position"], x["out_Quantity.quantity"], unit) for x in period["Point"]]
